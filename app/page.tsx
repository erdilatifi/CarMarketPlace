...
const HomePage = () => {
  ...
  const { data: brands = [], isLoading: loadingBrands } = useQuery({
    ...
    queryFn: async () => {
      ...
      const unique = Array.from(
        new Set((data ?? []).map((r: any) => r.brand).filter(Boolean))
      ) as string[];
  ...
  // Favorites
  const { data: favoritesSet = new Set<string>(), isLoading: loadingFavs } =
    useQuery({
    ...
    <div className="min-h-screen w-full pt-6 md:pt-8 px-6 pb-10">
      <div className="max-w-7xl mx-auto">
        ...
        <form
          ...
          className="bg-black/40 backdrop-blur rounded-2xl border border-white/10 p-8 grid grid-cols-1 md:grid-cols-2 gap-6"
        >
          {/* Brand Select */}
        ...
        <div className="mt-8">
          {loadingCars || loadingFavs ? (
            ...
            <p className="text-muted-foreground text-center">No cars found.</p>
          ) : (
            <>
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {currentItems.map((car) => {
                  ...
                  return (
                    ...
                    >
                      <Card className="flex flex-col rounded-2xl bg-gradient-to-b from-[#121212] to-[#161616] hover:shadow-[0_0_0_1px_rgba(255,255,255,0.08),0_18px_50px_-12px_rgba(0,0,0,0.7)] transition overflow-hidden ring-1 ring-white/5">
                        {/* Favorite */}
                        <button
                          type="button"
                          aria-label="favorite"
                          ...
                          className={`absolute top-3 right-3 z-10 p-2 rounded-full shadow-sm transition border border-white/10 ${
                            isFav
